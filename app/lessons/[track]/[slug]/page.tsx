...
export default async function LessonDetailPage({
  ...
  try {
    content = await readFile(`${track}/lessons/${slug}.md`);
  } catch (e) {
  ...
  return (
    <div>
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-zinc-900/95 backdrop-blur border-b border-zinc-200 dark:border-zinc-800 px-4 py-3 flex items-center gap-3">
        <Link href="/lessons" className="text-blue-600 dark:text-blue-400">←</Link>
        <div className="min-w-0">
          <p className="text-xs text-zinc-500 capitalize">{track}</p>
